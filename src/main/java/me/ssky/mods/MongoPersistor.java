...
public class MongoPersistor extends BusModBase implements Handler<Message<JsonObject>> {
	...
	private void doSaveAll(Message<JsonObject> message) {
		String collection = getMandatoryString("collection", message);
		if (collection == null) { return; }
		String docs = getMandatoryString("document", message);

		JsonArray array = new JsonArray(docs);
		Iterator<Object> itr = array.iterator();
		JsonArray reply = new JsonArray();
		while (itr.hasNext()) {
			JsonObject doc = (JsonObject) itr.next();
			if (doc == null) return;
		...
		sendOK(message, new JsonObject().putArray("results", reply));
	}
	...
	private void doFind(Message<JsonObject> message) {
		...
		DBCollection coll = db.getCollection(collection);
		DBCursor cursor = (keys == null) ? coll.find(jsonToDBObject(matcher)) : coll.find(jsonToDBObject(matcher), jsonToDBObject(keys));
		if (skip != -1) {
			cursor.skip(skip);
		}
		if (limit != -1) {
	...
	private DBObject sortObjectToDBObject(Object sortObj) {
		if (sortObj instanceof JsonObject) {
			...
			for (Object curSortObj : sortJsonObjects) {
				if (!(curSortObj instanceof JsonObject)) { throw new IllegalArgumentException("Cannot handle type " + curSortObj.getClass().getSimpleName()); }
